...
function isValidMapping(value) {
  return (
    typeof value === "object" &&
    value !== null &&
    typeof /** @type {Record<string, unknown>} */ (value).envFilePath === "string" &&
    typeof /** @type {Record<string, unknown>} */ (value).secretName === "string" &&
    /** @type {Record<string, unknown>} */ (value).envFilePath.length > 0 &&
    /** @type {Record<string, unknown>} */ (value).secretName.length > 0
  );
}
...
export function loadConfigFile(filePath) {
  ...
  console.log(`Loading config file: ${resolved}`);
  ...
  const mappings = /** @type {Record<string, unknown>} */ (parsed).mappings;
